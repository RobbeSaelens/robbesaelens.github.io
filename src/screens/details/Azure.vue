    ...
    <div class="mx-auto max-w-screen-xl px-4 pt-10 sm:px-6">
      <header class="text-center">
        <h1 class="terminal-title justify-center text-2xl font-bold sm:text-3xl lg:text-5xl">
          ...
          <span class="terminal-text">{{ $t('azure.title') }}</span>
          <span class="terminal-cursor" aria-hidden="true"></span>
        </h1>
        <p class="terminal-subtitle mt-3 text-sm tracking-wide md:text-base">
          <span class="opacity-50">$</span> cat ./azure-project.md
        </p>
        <div class="mt-4 flex flex-wrap justify-center gap-2">
          <span class="tag-pill">{{ $t('azure.subtitle1') }}</span>
          <span class="tag-pill">{{ $t('azure.subtitle2') }}</span>
          <span class="tag-pill">{{ $t('azure.subtitle3') }}</span>
        </div>
      </header>
      <div>
        <img
          class="mx-auto hidden md:block"
          src="/ResearchAzureMockup.png"
          alt="Azure Search project mockup showing Netflix-style movie search interface"
          loading="lazy"
        />
        <img
          class="h-120 my-5 mx-auto block md:hidden"
          src="/AzureIphone.png"
          alt="Azure Search project mobile mockup on iPhone"
          loading="lazy"
        />
      </div>
      ...
      <div class="md:mx-10 mx-5 pt-10">
        <!-- Section Header -->
        <div class="mb-10">
          <h2 class="text-4xl md:text-5xl font-bold mb-3" style="color: var(--color-text-primary)">
            {{ $t('azure.overview') }}
          </h2>
        </div>

        <!-- About + iPhone Image Row -->
        <div class="flex flex-col md:flex-row gap-8 mb-8">
          <div class="overview-card flex-1 px-6 py-8 md:px-8">
            <h3 class="overview-label">{{ $t('azure.about') }}</h3>
            <p class="leading-relaxed text-lg" style="color: var(--color-text-secondary)">
              {{ $t('azure.aboutText') }}
            </p>
            <h3 class="overview-label mt-6">{{ $t('azure.languages') }}</h3>
            <div class="flex flex-wrap gap-2">
              <span class="tag-pill">Amazon OpenSearch</span>
              <span class="tag-pill">Azure Cognitive Search</span>
              <span class="tag-pill">Vue.js</span>
              <span class="tag-pill">TailwindCSS</span>
            </div>
            <h3 class="overview-label mt-6">{{ $t('azure.tools') }}</h3>
            <div class="flex flex-wrap gap-2">
              <span class="tag-pill">Visual Studio Code</span>
              <span class="tag-pill">Azure Portal</span>
              <span class="tag-pill">Amazon AWS</span>
              <span class="tag-pill">Adobe XD</span>
            </div>
          </div>
          <div class="hidden md:flex items-center justify-center flex-shrink-0">
            <img
              class="w-56"
              src="/AzureIphone.png"
              alt="Azure Search app iPhone mockup"
              loading="lazy"
            />
          </div>
        </div>
        ...
        <!-- View Other Projects -->
        <div class="mt-4 inline-block" style="color: var(--color-accent)">
          <router-link class="flex items-center gap-2 group" to="/projects">
